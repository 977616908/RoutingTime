...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            PSLog(@"Prefetched %d out of %d", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
            PSLog(@"Prefetched %d out of %d (Failed)", self.finishedCount, [self.prefetchURLs count]);
...
- (void)reportStatus
{
    [self.prefetchURLs count];
//    PSLog(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
